#!/usr/bin/env python
"""
Per-dataset BumpNet-yield measurement -- per-job driver.

One job = one file from one CMS Open Data primary dataset or MC process.
Measures the SAME 186-combination-pattern set, the SAME shared
mass-calculation/naming/post-processing code as
studies/cms_coverage/cluster/run_coverage_on_file.py -- but with two
deliberate, task-scoped changes to the EVENT SELECTION front-end:

  1. NO trigger requirement, on ANY dataset (including DoubleMuon) --
     apply_trigger() is never called. Object definitions themselves are
     unchanged from studies/m0m1j0_cms/RECIPE.md (imported directly:
     selection.select_muons / select_electrons / select_and_split_jets).
  2. NO topology requirement (no ">=2 muons and >=1 jet" gate) -- an
     event is kept if it has >=2 selected objects of ANY type (muon,
     electron, jet, or b-jet), summed.

Golden-JSON (validated-runs) filtering is applied to DATA files only
(--is-mc unset); never to MC (--is-mc set), matching the task's own
instruction and CMS convention (a simulated run has no real
run/luminosityBlock certification to check).

Reads the full file in CHUNKS (not one whole-file request) -- the fix
already established in studies/cms_coverage/cluster/run_coverage_on_file.py
after a whole-file read failed reproducibly on a real file there.

Storage: same per-job SQLite shard format as the DoubleMuon combination
survey (services.storage.sqlite_shards.SqliteArrayShardWriter), so the
existing merge/post-processing code
(studies/cms_coverage/cluster/merge_and_count.py's
run_funnel_at_threshold, itself calling the real, unmodified
...
_find_rightmost_highest_peak / _split_by_first_empty_bin) can be reused
directly, unmodified, per dataset.

Usage:
    python run_per_dataset_on_file.py --record-id 30525 --file-index 0 \
        --output-dir /storage/.../job_JetHT --dataset-label JetHT
    python run_per_dataset_on_file.py --record-id 67801 --file-index 0 \
        --output-dir /storage/.../job_ttbar --dataset-label mc_ttbar --is-mc
"""
...
# Object definitions themselves are UNCHANGED from RECIPE.md -- these are
# the exact branches selection.py's own select_muons/select_electrons/
# select_and_split_jets read. Trigger branches are deliberately excluded
# (no trigger requirement applied here, on any dataset), so this list is
# NOT selection.NEEDED_BRANCHES verbatim (that constant includes
# DoubleMuon-specific HLT branches that other primary datasets don't have).
REQUIRED_BRANCHES = (
...
MIN_TOTAL_SELECTED_OBJECTS = 2  # the dropped-topology requirement: >=2 objects of ANY type
...
def read_events(file_url: str):
    """Chunked read (see module docstring) -- reading REQUIRED_BRANCHES
    only (no trigger branches: no dataset-specific HLT path is required
    or read, since no trigger requirement is applied anywhere in this
    task)."""
...
def main():
    ...
    # NO trigger requirement: selection functions are called directly on
    # `events` (post golden-JSON for data, as-read for MC), never on a
    # trigger-filtered subset.
    ...
    # DROPPED topology requirement: keep an event if it has >=2 selected
    # objects of ANY type (muon, electron, jet, or b-jet), not specifically
    # ">=2 muons and >=1 light jet".
    ...
    writer.set_metadata("trigger_applied", False)
