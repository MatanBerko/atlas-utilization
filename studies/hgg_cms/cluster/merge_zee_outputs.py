...
Simpler than merge_outputs.py: both Z->ee jobs (data, DY -- REVISED
TWICE, 16 Sep 2026: first from 4 variants down to 2 when the "trigger
-eff" sample became an offline cut instead of a separate cluster job;
then the data variant switched from DoubleEG to SingleElectron records,
see config.cms_hgg_zee_data.yaml's own comment) use the SAME one-file-
per-job array-job layout (pbs_hgg_zee_array.sh), so there is only one
mode here, not a data/signal split. Same verified identity-check method
as merge_outputs.py's data mode: each job's own metadata_cache.json +
its array index, sliced with the real utils.batching.get_batch_slice_by_year,
checked against a frozen expected file list --
cms_zee_singleelectron_data_file_lists.json (151 SingleElectron files,
30529 Run2016G + 30562 Run2016H) for the data variant,
cms_zee_dy_file_list.json (41 DY files) for the DY variant. No blinding
logic anywhere in this script -- the Z->ee output has no blinded window
(studies/hgg_cms/zee_output.py).

Usage:
    python merge_zee_outputs.py --total-batches 151 \
