...
* Checks the overall scale of the numbers. The pipeline multiplies every
  invariant mass by 1e-3 in services/pipelines/im_pipeline.py
  (`_convert_array_to_gev`). That is right for ATLAS PHYSLITE (MeV inputs) but
  wrong for CMS NanoAOD, whose pt/mass are already in GeV - so the CMS masses
  land near 0.09 instead of 91. If the reference (electron) channel looks
  scaled down by a round power of ten, this script rescales BOTH channels by
  that factor and shouts about it, so the physics check still means something.
* Bins the (rescaled) values 60-120 GeV in 2 GeV steps, writes a bar-chart PNG
  next to the data.
* Prints, in plain text: how many pairs were found for each channel, which
  2 GeV bin in the 85-97 GeV window holds the most entries (the candidate Z
  peak), and a short plain-language verdict.
...
-----
    ...
    python scripts/zpeak_check.py --no-autoscale  # do not rescale, take values as-is
"""
...
def infer_pipeline_scale(ref_vals: np.ndarray):
    """
    Work out whether the pipeline's invariant masses are on the GeV scale or
    off by a round power of ten. Uses the electron-pair channel as reference
    (high stats, dominated by real Z->ee). Returns (factor, explanation).
    """
    v = ref_vals[np.isfinite(ref_vals)]
    v = v[v > 0]
    if v.size < 20:
        return 1.0, "not enough electron-pair values to judge the scale; taking numbers as-is (assumed GeV)."
    med = float(np.median(v))
    p = int(round(math.log10(Z_MASS / med)))
    factor = 10.0 ** p
    if p == 0:
        return 1.0, f"electron-pair median is {med:.3g} GeV - already on the GeV scale, no rescale needed."
    return factor, (
        f"electron-pair median is {med:.4g}, about {factor:g}x below the Z mass. "
        f"The pipeline's invariant masses for this CMS run are therefore a factor of "
        f"{factor:g} too small - a unit-conversion bug (im_pipeline._convert_array_to_gev "
        f"multiplies by 1e-3, which only makes sense for MeV inputs; CMS NanoAOD is "
        f"already in GeV). This check rescales BOTH channels by x{factor:g}."
    )
...
def verdict(values: np.ndarray, edges: np.ndarray) -> str:
    ...
    if peak_ok and (contrast >= 2.0 or peak_frac >= 0.15):
        return (f"CLEAR Z peak at {lo:.0f}-{hi:.0f} GeV "
                ...
                f"(~{Z_MASS:.1f} GeV): the CMS parsing and mass calculation are working "
                f"for this channel (modulo the overall scale factor noted above).")
    if peak_ok:
...
def main() -> int:
    ...
    ap.add_argument("--no-autoscale", action="store_true",
                    help="Do not rescale; bin the raw pipeline values as-is")
    args = ap.parse_args()
    ...
    # --- decide on a global scale factor from the electron-pair channel ---
    if args.no_autoscale:
        factor, scale_note = 1.0, "autoscale disabled (--no-autoscale): using raw pipeline values."
    else:
    ...
    print(f"  {scale_note}")
    if factor != 1.0:
        print(f"  --> applying x{factor:g} to every mass value below.")
    print()
    ...
    for ax, (im_str, label) in zip(axes, CHANNELS):
        all_vals = data[im_str]["all"] * factor
        pure_vals = data[im_str]["pure"] * factor
        all_in = all_vals[(all_vals >= BIN_LO) & (all_vals <= BIN_HI)]
        pure_in = pure_vals[(pure_vals >= BIN_LO) & (pure_vals <= BIN_HI)]

        print("-" * 74)
        print(f"CHANNEL: {label}   (IM signature '{im_str}')")
        print(f"  total pairs found (all final states) : {all_vals.size}")
        print(f"    in {BIN_LO:.0f}-{BIN_HI:.0f} GeV after x{factor:g}       : {all_in.size}")
        print(f"  pairs in the pure '{PURE_FS[im_str]}' final state : {pure_vals.size}")
        if pure_vals.size:
            print(f"    in {BIN_LO:.0f}-{BIN_HI:.0f} GeV                       : {pure_in.size}")

        pk_all = peak_bin(all_vals, edges)
        if pk_all:
            print(f"  candidate Z bin (all final states)   : "
                  f"{pk_all[0]:.0f}-{pk_all[1]:.0f} GeV  with {pk_all[2]} entries")
        else:
            print(f"  candidate Z bin (all final states)   : none in "
                  f"{Z_WINDOW[0]:.0f}-{Z_WINDOW[1]:.0f} GeV")
        pk_pure = peak_bin(pure_vals, edges)
        if pk_pure:
            print(f"  candidate Z bin (pure final state)   : "
                  f"{pk_pure[0]:.0f}-{pk_pure[1]:.0f} GeV  with {pk_pure[2]} entries")

        print(f"  interpretation: {verdict(all_vals, edges)}")
        print()

        counts, _ = np.histogram(all_in, bins=edges)
        centers = 0.5 * (edges[:-1] + edges[1:])
        if all_in.size:
            any_data = True
        ...
        scale_txt = "" if factor == 1.0 else f"  (raw x{factor:g})"
        ax.set_title(f"{label}{scale_txt}\n{all_in.size} entries in {BIN_LO:.0f}-{BIN_HI:.0f} GeV")
        ax.set_xlabel("invariant mass [GeV]")
        ax.set_ylabel("pairs / 2 GeV")
        ax.legend(loc="upper right", fontsize=9)

    sup = "CMS di-lepton invariant mass - Z-peak check"
    if factor != 1.0:
        sup += f"   [pipeline values rescaled x{factor:g}]"
